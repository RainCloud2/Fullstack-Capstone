...
function buildProfileText(a) {
  // Kita translasikan maknanya ke Bahasa Inggris agar SBERT paham
  const goalMap = {
    software: "become a software engineer",
    upgrade: "upgrade my current tech skills",
    freelance: "build freelance programming projects",
    explore: "explore and learn coding basics",
  };

  const levelMap = {
    beginner: "a beginner",
    basic: "at a basic level",
    intermediate: "at an intermediate level",
    advanced: "at an advanced level",
  };

  const interestMap = {
    frontend: "frontend web development, user interfaces, and HTML/CSS/JS",
    backend: "backend development, APIs, servers, and databases",
    ai: "artificial intelligence, machine learning, and data science",
    mobile: "mobile app development for Android and iOS",
  };

  // Hasilkan kalimat cerita utuh (Semantic Context) untuk AI
  return `I want to ${goalMap[a.goal]}. I am currently ${levelMap[a.level]}. I am highly interested in learning about ${interestMap[a.interest]}.`;
}
...
export default function OnboardingPage() {
  ...
  const handleNext = () => {
    ...
    const payload = {
      ...answers,
      pretest_profile_text: buildProfileText(answers),
    };
    ...
    <div className="page-shell min-h-screen">
      <header className="border-b border-emerald-950/10 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
          <div>
            <p className="font-display text-2xl text-[#172017]">
        ...
        <div className="h-1.5 bg-slate-100">
          <div
            className="h-full bg-[#4d8b41] transition-all"
            style={{ width: `${(step / data.length) * 100}%` }}
          />
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:px-8">
        <aside className="section-card h-fit p-6">
          <p className="text-sm font-bold uppercase tracking-[0.22em] text-[#4d8b41]">
            Personalized path
          </p>
          <h1 className="font-display mt-3 text-4xl leading-tight text-[#172017]">
            Biar AI menyusun jalur yang masuk akal.
          </h1>
          <p className="mt-4 leading-7 text-slate-600">
            Jawabanmu akan dipakai untuk menentukan level awal, rekomendasi course,
            dan ritme belajar harian.
          </p>
        ...
        <section className="section-card p-5 sm:p-8">
          ...
          <h2 className="font-display mt-3 text-4xl leading-tight text-[#172017]">
            {current.title}
          </h2>
                ...
                >
                  ...
                  <span>
                    <span
                      className={[
                        "block font-bold",
                        active ? "text-[#285b2f]" : "text-[#172017]",
                      ].join(" ")}
                    >
                      {title}
                    </span>
